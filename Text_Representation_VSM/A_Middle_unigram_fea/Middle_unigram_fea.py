...
def Middle_unigram_fea(upper_path_pre, upper_path_VSM):											#incoming parameter is the path of preprocess result and the path of VSM result
	print '\n*********************************************************************\n'
	print 'start constructing feature vectors using middle unigram as feature term...\n'
	...
	for direct1 in sorted(os.listdir(upper_path_pre)):
		...
		path_fea_term_set, path_mid_uni_sin, path_mid_uni_mul = Extract_feature_term.Extract_feature_term(path_sin, path_mul, upper_path_VSM+'/'+direct1)
		...
		#Construct_feature_vector.Construct_feature_vector(path_fea_term_set, path_mid_uni_sin, path_mid_uni_mul)
		...
		for direct2 in filter(lambda x: os.path.isdir(upper_path_pre+'/'+direct1+'/'+x), os.listdir(upper_path_pre+'/'+direct1)):
			...
			for direct3 in sorted(os.listdir(upper_path_pre+'/'+direct1+'/'+direct2)):
				...
				path_fea_term_set_tra, path_mid_uni_sin_tra, path_mid_uni_mul_tra = Extract_feature_term.Extract_feature_term(path_sin_tra, path_mul_tra, lower_path_VSM)

				#Construct_feature_vector.Construct_feature_vector(path_fea_term_set, path_mid_uni_sin_tra, path_mid_uni_mul_tra)
...
if __name__ == '__main__':
	print 'debugging...' 
	path_sin = 'Preprocess_results/Middle_unigram/0/Middle_unigram_0.txt' 	
	path_mul = 'Preprocess_results/Middle_unigram/0/Middle_unigram_0-5.txt'
	lower_path_VSM = 'VSM_results/Middle_unigram/0'
